...
///
/// **接在評分鏈的單一入口**（`batch_data_loader` 組 `pricesMap` 處，isolate 與
/// 主執行緒兩條路徑共用）。曾一度因「會重現自選股空白卡」而撤回，2026-08-30
/// 對**實際自選清單**量測後推翻：43 檔裡只有 2603 有斷點，截斷後仍有 51 根、
/// 不會被 21 根閘擋下；當天會掉列的 17 檔沒有一檔在清單裡。
///
/// 為什麼不接在指標函式裡（「標記斷點、讓跨越它的長窗指標回 null」）：那是
/// **12 個履行點**（11 個規則檔各自讀 `data.prices` 算自己的窗 +
/// `calculateTechnicalIndicators`），而入口只有 1 個。
///
/// 殘餘風險有界且可見：除息旺季會有自選股落進「截斷後不足 21 根」的約一個月
/// 視窗，屆時卡片會少掉趨勢與評分。loader 對此單獨記一行 log（那個成因原本
/// 混在 `insufficientData` 裡，與「新上市」分不開）。
///
