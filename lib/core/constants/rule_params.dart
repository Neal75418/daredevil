...
abstract final class RuleParams {
  ...
  /// 判定「價格水位斷點」的相鄰有效收盤變動比（12%）
  ///
  /// `daily_price` 存的是交易所原始收盤價、**未還原除權息／減資／分割**，
  /// 跨越水位位移的長窗指標會產出物理上不可能的值（5904 寶雅 2026-08-27 的
  /// `ma60 = 506.18`，而當天股價 74.10）。超過本比值的相鄰變動視為斷點，
  /// 分析只用斷點之後的區段（見 `PriceContinuity.contiguousSuffix`）。
  ///
  /// **門檻依交易所規則 + 實測分布**：台股日漲跌幅上限 ±10%，跳動單位
  /// 四捨五入使實測分布延伸到約 11%，之後出現斷崖——本機 DB（2025-06-10～
  /// 2026-08-28，300 個交易日）實測 10–11% 有 1,298 筆、11–12% 只有 62 筆
  /// （20 倍落差），12% 取在斷崖之後。
  /// 全庫 ≥12% 共 189 筆 / 165 檔。
  static const double priceDiscontinuityRatio = 0.12;
  ...
  ///
  /// ⚠️ **與已移除的候選股數門檻同一個 bug class**（2026-08-29 稽核 C1 掃 sibling
  /// 時發現，刻意未動）：股數門檻對高價股綁得最緊。移除候選層門檻後新放行的
  /// 高價薄量族群在數學上永遠達不到 1,000 萬股——5274 信驊實測日成交 345,184
  /// 股——於是它們進得了 scoring 卻對 `DayTradingHighRule` 結構性免疫。
  /// 這條與下面兩條要不要改是**獨立決策**，需要各自的量測（當沖比率的意義
  /// 與流動性不同，股數在這裡未必是錯的代理），不隨候選層那次一併移除。
