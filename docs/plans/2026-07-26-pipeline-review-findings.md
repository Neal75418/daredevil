...
> **後續（2026-08-29 晚，領域稽核 C1）**：上面第 2 道閘裡的「股數 ≥ 100 萬」
> 已**整條移除**。它與成交額門檻同時要求時，對股價 p 的股票等於要求
> `量 ≥ max(100萬, 3000萬/p)`——p 超過 30 元後綁定的永遠是股數那條，實際
> 成交額門檻因此變成 `p × 100 萬`（15,630 元的信驊要 156 億）。實測它擋掉
> 68.9% 的 stock-day，其中 25.7% 已通過成交額門檻。單日閘現在純看成交額，
> 與中位數閘同口徑、只差窗長；log 上單日閘的額外剔除由 11.4% 降到 4.2%。
> **本段以下的敘述是 2026-08-29 白天的狀態，刻意不改寫。**
