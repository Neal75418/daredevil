...
- 交易日曆錯 23 天，以證交所公告與每日成交資訊（2024-01～2026-09 共 662 個交易日）逐日核對後修正：
  ...
  - 2025：漏列春節前無交易 2 天；把 2/3、2/4、6/2、10/7 誤列為休市——缺漏偵測因此從未回補 2025-10-07（資料庫該日僅 55 檔價格）；
    修正後該日落在回補窗內、會被列為缺漏日補抓
  - 2027（依萬年曆推估）：漏列小年夜、兒童節與勞動節補假、中秋（誤植為 10/15）、教師節、光復節、行憲紀念日與元旦補假，
    另把 2/11、10/15 誤列為休市。改依人事行政總處 116 年辦公日曆表；證交所春節前另休的日子待證交所公告後補上
  - 漏列休市日會讓歷史價格回補把它當缺漏日反覆重抓（2025 年新增假日時發生過）；多列則讓真正的缺漏永遠不回補
- 今日訊號分頁沒有訊號或載入失敗時，族群排行與財報入口被推到第一屏之外：空狀態撐滿剩下的整個畫面、本身又約 336 高。
  改為依內容高度顯示的精簡版（圖示與間距縮小，文字與按鈕不變），390×844 畫面下族群排行標題在第一屏內
- 內部人轉讓在「兩種轉讓方式並存」時股數錯誤（3189 景碩顯示 80000008000 張、2442 顯示 0）：官方把兩個方式、兩個股數
  接在同一格，無分隔時兩個數字被接成一個、以空格相隔時解析失敗成 0。改以「預定轉讓總股數」（自有＋信託，各為單一數字）
  為準；讀出比目前持股還多的股數視為格式異常、整筆跳過並記 warning。舊規則寫入的這類錯誤列（官方不會重給、個股詳情的
  內部人分頁不限日期）於同步時清除
- 籌碼異動「法人集中買賣」賣超也顯示紅色：數值原本一律依嚴重度上色，改依買賣方向（買超紅、賣超綠，同頁法人動向慣例）；
  質押率、融券倍數等風險指標維持嚴重度色
- 今日訊號分頁漏掉強訊號、混進「觀察」卡：分派先依模式優先順序選、選完才檢查分數，起漲模式分數偏低的股票會擋掉
  它在強勢模式的訊號、整檔不出現（例：2454 強勢 38 分），被選中的模式不到門檻時則顯示「觀察」。改為先篩出分數本身
  合格的模式再依優先順序挑（2026-09-24 資料：強勢分頁候選 55 → 60 檔，起漲、回檔不變）

- 手機寬度下族群排行的期間切換把第一個選項「今日」擠出畫面：標題與切換鈕平分寬度、切換鈕在靠右捲動裡。
  改為空間不夠時整排移到標題下一行、所有選項都看得到；寬螢幕時切換鈕改為靠右（原本停在中間）

### Changed

- 今日、掃描、自選分頁開啟的底部面板（長按預覽、篩選、說明、群組管理等）改為蓋住整個畫面、包含底部導覽列（平板與桌面為
  側邊導覽欄），與「更新紀錄」一致。
  原本停在導覽列上方，開著也能切分頁，切回來時面板還開著。所有面板改由同一個函式開啟，守門測試禁止直接呼叫原生 API
- 交易日曆需要更新時（12 月起下一年未經證交所確認、或今年本身未確認），更新紀錄、更新摘要與手機背景更新通知都附
  「交易日曆待更新」，不算更新失敗；新增 `tool/check_twse_holidays.dart` 以證交所官方 API 逐日核對休市日
- 回檔分頁不再顯示強／中／弱（卡片徽章、長按預覽的評分區、朗讀的分數）：回檔的四種條件擇一成立、幾乎每檔只觸發一條，
  近 99% 的卡片都是「弱」，分級分不出差別又容易被讀成弱訊號；哪一種回檔看卡片標籤。「訊號卡怎麼看」新增回檔分頁的說明
- 長按預覽的評分改用卡片同一個分級徽章：原本等級字（80/60/40）、圓環色（50/35/20，高分用紅色）與卡片的強／中／弱
  （45/25/12）三套標準，同一檔股票長按前後顯示不同等級；深色主題下分數 ≥ 80 的卡片也不再另換漸層底，強弱只看徽章
- 訊號卡看得懂：分數徽章改用品牌藍深淺（強＝實心、中＝外框、弱＝灰框、觀察＝無框），不再用綠系——緊貼綠色下跌數字時會被讀成看跌；
  徽章旁只顯示分級依據的那個分數（5日、60日較高者），移除 8px 的「5D／60D」雙數字——並排時窄卡片會把徽章整顆縮到看不清；
  走勢小圖顏色改依畫出來那段（最近 20 筆收盤）自己的漲跌，原本跟著今日漲跌、會出現「線往上卻是跌色」。掃描、自選頁共用同一張卡
- 今日訊號標題旁新增 ⓘ「訊號卡怎麼看」：說明強／中／弱／觀察門檻、5日／60日兩套權重、分數差異沒有統計意義，以及箭頭、走勢圖、收盤與漲跌各代表什麼
...
- FinMind token 改以 `Authorization: Bearer` header 傳送，不再出現在 URL；日誌與 Sentry
  上報前遮掉 token 與 SQL 綁定參數（持倉、備註）

### Fixed

- 設定頁存／清 FinMind token 或改快取時間後，連線退回匿名模式（額度 600→300/hr）直到重開 app
- macOS 回到視窗從不重新載入；launchd 在 App 外完成的更新也不會反映到各頁（今日頁日期變新、訊號清單仍是舊的）
- 開 App 時資料已是最新仍跑整輪更新，交易日早上白燒 FinMind 額度；改為資料落後交易日才自動更新
- macOS 最小化或 Cmd+Q 結束時沒有保存 API 配額計數，重啟後少算、可能撞上服務端限流
- 今日頁下拉只重讀資料庫、不抓新資料；改為仍落後才更新、已是最新則提示。重新載入時整頁不再閃成骨架
- 提醒建立時預填的現價會跟著切換類型被存成門檻（質押比 ≥850% 永不觸發、PE ≤850 幾乎天天觸發）；
  切換類型改重設為該類型預設、不需目標值的類型存 0 並隱藏欄位、編輯時鎖定類型。
  質押與爆量提醒改用固定門檻，既有資料裡殘留的現價不再影響判斷
- 「成交量高於 N 張」實際拿股數比較（輸入 1 萬張＝比 10 張，幾乎天天觸發），改以張計
- TDCC 股權分散表的「本週已同步」判斷在平日永遠不成立，每輪更新重抓約 9.8 MB；改為最新一期未滿 7 天就跳過
- 行事曆刪除後的「復原」按鈕顯示 `common.undo` 字面
- 財報回填的上市名額窗按**波動度**分配，低波動大型股永遠排不進去——app DB 實查
  460 檔只有官方批次給的資產負債表、零 EPS，其中 18 檔當天有評分且全是上市大型
  低波動股（台灣大、群光、佳世達、統一證、統一超…）。7 條 EPS／ROE 規則對它們無聲
  不觸發，而 9/04 觸發至少一條這類規則的 235 檔平均從中拿到 short 16.4／long 21.2
  分（這 235 檔的平均總分 24.8／35.8）；五輪日誌實測被回填的 102 檔裡 0 檔來自那 460 檔。原本是兩條佇列
  （上市取候選前 150、上櫃最舊優先）＋「上市先拿、上櫃吃剩」的配額拆分；分開的唯一
  理由是上市名額窗把上櫃餓死，而最舊優先下餓死在結構上不可能發生。收成**單一
  「最舊優先」佇列**：自選＋熱門優先，其餘全市場先剔 ETF、再依 INCOME 最新日期
  由舊到新取到上限 200（＝整點滿額度時 reserve 公式的值）。額度感知與 reserve 200
  原樣保留；穩態下選到的都是新鮮股、被新鮮度過濾成零呼叫，長期用量不變。
  行為差異：自選＋熱門現在**計入上限**（舊碼讓它們永遠全員入列、不受額度限制），
  額度吃緊到上限小於 priority 數時先截熱門股、自選股保住。移除已死的
  `otcFinancialLimitForBudget` 與 `otcFinancialBackfillReserve`；上市的
  `financialSyncMaxCandidates` 與上櫃的 `otcFinancialSyncMaxCount` 由單一
  `financialSyncMaxCount` 取代
...
- 今日頁在資料落後交易日時顯示「資料落後 N 個交易日（最新 M/D 收盤）」與「立即更新」；週末、連假不算落後
- 全新安裝顯示「尚未有資料／正在建立資料…」而非「今日無訊號」；歷史未補齊期間顯示
  「歷史資料建置中 N%（約再 M 次更新補齊）」，提醒長週期訊號可能尚未出現
- 首次使用（含既有安裝）需閱讀並同意免責聲明；今日清單底部常駐短版聲明，「關於」附全文與資料來源顯名
...
- 文案中性化：「今日推薦／強烈推薦」改為「今日訊號／訊號強度」，移除「進場、分批進場、停利停損、建議觀望、
  反彈機會、追高、接刀」等操作指示；「AI 智慧分析」改為「規則摘要」（摘要本就是規則模板）
- Android release 在 CI 缺 keystore 時直接失敗，不再產出 debug 簽章的 APK（無法覆蓋升級）；
  發佈流程改為先通過完整 CI，並保存混淆符號檔
...
- 技術指標（RSI／KD／MA）只用**最後一個價格水位斷點之後**的資料——部分股票的
  MA60 會變成 null。`daily_price` 存交易所原始收盤價、未還原除權息／減資／分割，
  跨越位移的長窗指標會給出物理上不可能的值：5904 寶雅 2026-08-27 的
  `daily_reason` 存著 `ma60: 506.18`，而當天股價 74.10；同檔在 15 個交易日裡
  有 12 天發出 `RSI_EXTREME_OVERSOLD {"rsi":17.4}`，以斷點後的資料重算約在 60。
  實測 165 檔有斷點，其中 **37 檔只失去 MA60**（其餘指標與 52 週規則完好）、
  15 檔斷點後不足 RSI 窗而整組指標為 null（那正是 5904 的形狀，null 比
  `ma60: 506` 誠實）。**52 週規則不受影響**——它本來就正確處理除息
  （`_sumDividendsInPeriod` 把窗內現金股利從極值扣掉）。
  門檻 12% 依交易所規則：台股日漲跌幅上限 ±10%，跳動單位四捨五入使實測分布
  延伸到約 11%，之後出現斷崖（10–11% 有 1,298 筆、11–12% 只剩 62 筆）
