...
- 指標只用**最後一個價格水位斷點之後**的歷史——**部分股票的均線類訊號會消失**。
  `daily_price` 存交易所原始收盤價、未還原除權息／減資／分割，跨越位移的長窗
  指標會產出物理上不可能的值：5904 寶雅 2026-08-27 的 `daily_reason` 存著
  `ma60: 506.18`，而當天股價 74.10。實測 2026-08-28 有 **14 檔**「當日有落庫
  評分、且 MA60 窗內含斷點」——其中 **2603 長榮在自選清單裡**，以
  `ma60: 209.79`（跨 6/17 除息 −12.0% 缺口）觸發 `PULLBACK_TO_MA20` 的 +15 分
  買進訊號；截斷後它只剩 51 根、MA60 變 null，該規則自己的守衛（`ma60 == null`
  就不觸發）終於生效，訊號停止。
  門檻 12% 依交易所規則：台股日漲跌幅上限 ±10%，跳動單位四捨五入使實測分布
  延伸到約 11%，之後出現斷崖（10–11% 有 1,298 筆、11–12% 只剩 62 筆）。
  比較相鄰的**有效收盤**，停牌不會被誤判——在此門檻下 176 筆永久位移只有 35%
  中間有交易日缺席，近三分之二（除權息）當天照常交易，靠缺口偵測抓不到。
  接在評分鏈的**單一入口**（兩條評分路徑共用），截斷後歷史不足門檻的股票由
  既有的 `insufficientData` 計數擋下，並單獨記錄成因（實測自選 43 檔中僅 2603
  有斷點且仍有 51 根，無一被擋下；除息旺季才會有股票落進約一個月的視窗）
